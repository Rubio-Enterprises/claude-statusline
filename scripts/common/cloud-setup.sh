...
if [ -n "$__smoke" ]; then
  echo "cloud-setup: SMOKE ($__smoke) — claude-statusline on $(uname -s)/$(uname -m); cloud-only sections skipped" >&2
else
  echo "cloud-setup: building claude-statusline environment cache (epoch ${CACHE_EPOCH:-unset})" >&2
fi
